...
const REVENUE_DATA = [
  ...
  { month: "Aug", revenue: 32 },
  { month: "Sep", revenue: 35 },
...
function useForm(defaults = {}) {
  ...
  const validate = (name, value, rules) => {
    ...
    if (rules.required && !value?.toString().trim())
      err = rules.required === true ? "This field is required" : rules.required;
    if (!err && rules.minLength && value?.length < rules.minLength)
      err = `Minimum ${rules.minLength} characters`;
    if (!err && rules.pattern && !rules.pattern.value.test(value))
      err = rules.pattern.message;
  ...
  const validateAll = (schema) => {
    ...
    Object.entries(schema).forEach(([name, rules]) => {
      let err = "";
      const v = values[name];
      if (rules.required && !v?.toString().trim())
        err = rules.required === true ? "Required" : rules.required;
      if (!err && rules.minLength && v?.length < rules.minLength)
        err = `Min ${rules.minLength} chars`;
      if (!err && rules.pattern && !rules.pattern.value.test(v))
        err = rules.pattern.message;
      if (err) {
        newErrors[name] = err;
        valid = false;
...
function AuthProvider({ children }) {
  ...
  return (
    <AuthCtx.Provider
      value={{ user, token, login, logout, isAuth, refreshUser, setUser }}
    >
      {children}
    </AuthCtx.Provider>
  );
  ...
  return (
    <div className="skel-card">
      ...
      <div className="skel-body">
        ...
        <div
          className="skeleton skel-line medium"
          style={{ height: 18, marginBottom: 14 }}
        />
  ...
  const html = `
    ...
    <body><div class="ticket">
      ...
      ${[
        ...
        .map(
          ([l, v]) => `
        <div class="row"><span class="label">${l}</span><span class="val">${v}</span></div>`,
        )
        .join("")}
      <div class="qr">
        <div>▓▓▓▓▓▓▓ ▓ ▓▓▓▓▓▓▓</div>
      ...
      <div className="card-body">
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          >
            ...
            <span style={{ color: pct > 85 ? "var(--red)" : "var(--text2)" }}>
              {event.seats - event.booked} left
            </span>
          </div>
...
function EventsPage({ setPage, setSelectedEvent }) {
  const { events, eventsLoading, eventsError, apiBase, refreshEvents } =
    useContext(EventCtx);
  ...
  const filtered = useMemo(() => {
    ...
    if (sort === "fill")
      res = [...res].sort((a, b) => b.booked / b.seats - a.booked / a.seats);
    return res;
  ...
  return (
    <div className="page section container">
      ...
      {eventsError && (
        ...
        >
          <div
            style={{ fontWeight: 700, marginBottom: 8, color: "var(--text)" }}
          >
          ...
          >
            ...
            <strong style={{ color: "var(--text2)" }}>{apiBase}</strong>
            {apiBase.includes("localhost:5000") && (
              <span>
                {" "}
                — this points at your machine, not the internet. Remove a wrong{" "}
                <code style={{ fontSize: 11 }}>VITE_API_URL</code> from the host
                so the app uses same-origin{" "}
                <code style={{ fontSize: 11 }}>/api</code> (with{" "}
                <code style={{ fontSize: 11 }}>vercel.json</code> / Netlify{" "}
                <code style={{ fontSize: 11 }}>_redirects</code>), or set{" "}
                <code style={{ fontSize: 11 }}>VITE_API_URL</code> to your live
                HTTPS API including <code style={{ fontSize: 11 }}>/api</code>.
              </span>
            )}
            {!apiBase.includes("localhost:5000") &&
              ...
              apiBase.startsWith(window.location.origin) && (
                <span>
                  {" "}
                  — same-origin <code style={{ fontSize: 11 }}>/api</code> is
                  used. Your host must proxy{" "}
                  <code style={{ fontSize: 11 }}>/api/*</code> to the Render API
                  (see{" "}
                  <code style={{ fontSize: 11 }}>frontend/vercel.json</code> and{" "}
                  <code style={{ fontSize: 11 }}>public/_redirects</code> in the
                  repo). Update the Render URL there if yours changed.
                </span>
              )}
          </p>
          <button
        ...
        >
          ...
          {eventsError ? (
            ...
            <>
              ...
              >
                If this is a new deployment, your production database may be
                empty. Seed events locally and export to Atlas, or set{" "}
                <code style={{ fontSize: 12 }}>SEED_ON_START=true</code> once on
                the API host (then turn it off).
              </p>
            </>
          ) : (
            <>
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const inList = evs.some(
          (e) => String(e._id ?? e.id) === String(event.id),
        );
      ...
      <div className="container">
        ...
        <div className="detail-grid">
          <div>
            ...
            >
              {!event.bannerUrl && <div>{event.emoji}</div>}
              {event.bannerUrl && (
                <div
                  style={{
                    position: "absolute",
            ...
            >
              {event.description?.trim() ? (
                ...
                <>
                  ...
                  . One of India&apos;s most anticipated events of the season,
                  bringing together the best talent, ideas, and entertainment
                  under one roof. Come be part of something extraordinary.
                </>
              )}
            </p>
            <h3
            ...
            <div className="sticky-card">
              ...
              <div className="info-row">
                ...
                <div>
                  ...
                  <div>
                    {event.seats - event.booked} / {event.seats} seats
                  </div>
                </div>
              </div>
              ...
              <button
                ...
                onClick={() =>
                  isAuth ? setPage("booking") : setModal("login")
                }
              >
                Book Tickets Now
              </button>
...
function BookingPage({ event, setPage, setModal }) {
  ...
  const { values, errors, register, validateAll } = useForm({
    name: user?.name || "",
    email: user?.email || "",
    phone: "",
  });
      ...
      <div className="container" style={{ maxWidth: 740 }}>
          ...
          <div>
            ...
            >
              <div className="flex-between mb-8">
                <span style={{ color: "var(--text2)" }}>Ticket Price</span>
                <span>
        ...
        {step === 2 && (
          <div>
            <div
              className="table-card"
              style={{ padding: 26, marginBottom: 20 }}
            >
...
function UserDashboard({ setPage, setModal }) {
  ...
  const totalSpent = bookings.reduce(
    (s, b) => s + (b.status !== "Cancelled" ? b.total : 0),
    0,
  );
            ...
            <div>
              <div
                style={{
                  ...
                  marginBottom: 32,
                  background:
                    "linear-gradient(135deg,rgba(232,184,109,0.08),rgba(78,203,141,0.04))",
                  border: "1px solid var(--border)",
                  borderRadius: var_radius(),
                  padding: 26,
                }}
              ...
              <div className="stats-row" style={{ marginBottom: 28 }}>
                ...
                />
                <StatCard
                  num={`₹${totalSpent.toLocaleString()}`}
                  label="Total Spent"
                  icon="💸"
                />
                <StatCard
                  num={String(wishlistEvents.length)}
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      >
                        ...
                        <div style={{ display: "flex", gap: 8 }}>
                          <button
                            className="btn btn-ghost btn-sm"
                            onClick={() => generatePDFTicket(b, user)}
                          >
                            📥 PDF
                          </button>
                          {b.status === "Confirmed" && (
                          ...
                          {b.status !== "Cancelled" && (
                            <button
                              ...
                              onClick={async () => {
                                try {
                                  await api(`/bookings/${b.mongoId}/cancel`, {
                                    method: "PATCH",
                                  });
                                  toast.add(
                                    "Booking Cancelled",
              ...
              >
                {wishlistEvents.length === 0 ? (
                  <p style={{ color: "var(--muted)" }}>
                    Your wishlist is empty. Browse events and tap ♡ on an event.
                  </p>
                ) : (
                  wishlistEvents.map((e) => (
                    ...
                    >
                      <div
                        style={{
                          height: 110,
                          background: e.bannerUrl
                            ? `url(${e.bannerUrl}) center/cover no-repeat`
                            : "var(--surface2)",
                          display: "flex",
                          alignItems: "center",
                          justifyContent: "center",
    ...
    <div style={{ maxWidth: 540 }}>
      ...
      >
        ...
        <div>
          ...
          <div
            style={{ color: "var(--text2)", fontSize: 13, marginBottom: 10 }}
          >
            Uploaded via Cloudinary · PNG/JPG, max 2MB
          </div>
...
function SettingsTab() {
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          height: 24,
          borderRadius: 12,
          background: toggles[name] ? "var(--accent)" : "var(--surface3)",
          border: "1px solid var(--border)",
          cursor: "pointer",
          position: "relative",
          flexShrink: 0,
      ...
      >
        <div
          style={{
            position: "absolute",
            top: 3,
            left: toggles[name] ? undefined : 3,
            right: toggles[name] ? 3 : undefined,
            width: 16,
            height: 16,
            borderRadius: "50%",
            background: toggles[name] ? "#0a0a0f" : "var(--muted)",
            transition: "all var(--transition)",
          }}
        />
      </div>
  ...
  return (
    <div style={{ maxWidth: 540 }}>
      ...
      <div
        className="table-card"
        style={{ padding: "0 16px", marginBottom: 28 }}
      >
        <T
          name="email"
        ...
        <T
          name="sms"
          label="SMS Alerts"
          desc="OTP and booking status via SMS"
        />
        <T
          name="remind"
          label="Event Reminders"
          desc="24h reminder before your event"
        />
        <T
          name="marketing"
          label="Marketing Emails"
          desc="Personalised event recommendations"
        />
        <T
          name="priceAlert"
          label="Price Drop Alerts"
          desc="Know when wishlist events go on sale"
        />
      </div>
      <h4
        style={{
      ...
      <div className="table-card" style={{ padding: 20, marginBottom: 28 }}>
        ...
        <button className="btn btn-primary btn-sm" onClick={updatePassword}>
          Update Password
        </button>
      </div>
      <div
      ...
      >
        <div className="fw-bold" style={{ marginBottom: 4 }}>
          Delete Account
        </div>
...
function OrganizerDashboard({ setPage }) {
  const { user, isAuth } = useContext(AuthCtx);
  const { organizerEvents, addEvent, updateEvent, deleteEvent } =
    useContext(EventCtx);
  const toast = useContext(ToastCtx);
  const [nav, setNav] = useState("overview");
  ...
  const { values, errors, register, validateAll, reset, setValue } =
    useForm(BLANK);
  ...
  const visibleEvents = myEvents.filter(
    (e) =>
      (filterCat === "All" || e.category === filterCat) &&
      (filterStatus === "All" || (e.orgStatus || "Active") === filterStatus),
  );
  const visibleAtt = ATTENDEES.filter(
    (a) =>
  ...
  const startEdit = (ev) => {
    ...
    setValue("price", String(ev.price));
    setValue("seats", String(ev.seats));
    setValue("description", ev.description || "");
    setValue("emoji", ev.emoji);
    setValue("status", ev.orgStatus || "Active");
    setValue("date", "");
    setValue("time", "");
    setBannerUrl(ev.bannerUrl || null);
    setNav("create");
  };
  ...
  const publish = async (draft = false) => {
    ...
    const dateStr = new Date(values.date).toLocaleDateString("en-IN", {
      day: "numeric",
      month: "short",
      year: "numeric",
    });
    ...
    try {
      if (editId) {
        ...
        toast.add(
          "Event Published! 🚀",
          "Your event is now live on Eventify",
          "success",
        );
      }
      reset();
      ...
      toast.add(
        "Save failed",
        e.message || "Check fields and try again",
        "error",
      );
    }
  };
    ...
    return (
      <div className="page section container" style={{ textAlign: "center" }}>
        ...
        <p style={{ color: "var(--text2)" }}>
          Login as an organizer to access this dashboard.
        </p>
      </div>
    );
    ...
    <div className="page" style={{ paddingTop: 68 }}>
      ...
      {confirmDel && (
        <div className="modal-overlay" onClick={() => setConfirmDel(null)}>
          ...
          >
            <div
              style={{ fontSize: 40, textAlign: "center", marginBottom: 14 }}
            >
              ⚠️
            </div>
            <div
        ...
        <div className="dash-content">
          {nav === "overview" && (
            <div>
              ...
              <div className="stats-row" style={{ marginBottom: 32 }}>
                ...
                />
                <StatCard
                  num={totalBooked.toLocaleString()}
                  label="Total Attendees"
                  icon="👥"
                  trend="12% growth"
                />
                <StatCard
                  num={"₹" + (totalRev / 100000).toFixed(1) + "L"}
                  label="Revenue"
                  icon="💰"
                />
                      ...
                      {visibleEvents.map((e) => {
                        ...
                        return (
                          <tr key={e.id}>
                            ...
                            >
                              ₹{(e.price * e.booked).toLocaleString()}
                            </td>
                            <td>
                              <Badge status={e.orgStatus || "Active"} />
                            </td>
                            <td>
                              <div
                                style={{
            ...
            <div style={{ maxWidth: 680 }}>
                  ...
                  {EMOJI_LIST.map((em) => (
                    <div
                      ...
                      style={{
                        ...
                        fontSize: 20,
                        cursor: "pointer",
                        border: `2px solid ${values.emoji === em ? "var(--accent)" : "var(--border)"}`,
                        background:
                          values.emoji === em
              ...
              >
                ...
                <div className="form-group">
                  ...
                  >
                    {values.description?.length || 0}/500 chars
                  </div>
                </div>
                <div
                  style={{
              ...
              >
                ...
                >
                  ...
                  <div className="form-group">
                    ...
                    <input
                      {...register("seats", { required: "Seats required" })}
                      type="number"
                      placeholder="200"
                      min="1"
                    />
                    {errors.seats && (
                      <div className="form-error">{errors.seats}</div>
                    )}
                  </div>
                  <div className="form-group">
                ...
                {bannerUrl ? (
                  ...
                  <label className="upload-drop" style={{ display: "block" }}>
                    ...
                    <div style={{ fontSize: 13, color: "var(--muted)" }}>
                      PNG, JPG — max 5MB · Uploaded to Cloudinary
                    </div>
                    <input
                      type="file"
                      accept="image/*"
          ...
          {nav === "attendees" && (
            <div>
              ...
              <div className="stats-row" style={{ marginBottom: 24 }}>
                ...
                />
                <StatCard
                  num={ATTENDEES.filter((a) => a.status === "Confirmed").length}
                  label="Confirmed"
                  icon="✅"
                />
                <StatCard
                  num={ATTENDEES.filter((a) => a.status === "Pending").length}
                  label="Pending"
                  icon="⏳"
                />
              </div>
          ...
          {nav === "revenue" && (
            <div>
              ...
              <div className="stats-row" style={{ marginBottom: 28 }}>
                <StatCard
                  num={"₹" + (totalRev / 100000).toFixed(2) + "L"}
                  label="Gross Revenue"
                  icon="💰"
                />
                <StatCard
                  num={"₹" + ((totalRev * 0.85) / 100000).toFixed(2) + "L"}
                  label="Your Earnings"
                  icon="🏦"
                />
              ...
              >
                ...
                {myEvents
                  .sort((a, b) => b.price * b.booked - a.price * a.booked)
                  .map((e) => {
                    const rev = e.price * e.booked;
                    const maxRev =
                      Math.max(...myEvents.map((x) => x.price * x.booked)) || 1;
                    return (
                      <div
                        key={e.id}
                        style={{
                      ...
                      >
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ fontSize: 12, color: "var(--muted)" }}>
                            {e.booked} tickets · ₹{e.price.toLocaleString()}
                          </div>
              ...
              >
                <div className="flex-between">
                  <div>
                    <div className="fw-bold mb-8">Next Payout</div>
                    <div style={{ color: "var(--text2)", fontSize: 13 }}>
                      Expected by Dec 25, 2025
                    </div>
                  </div>
                  <div
                    style={{
                  ...
                  >
                    ₹{((totalRev * 0.85) / 100000).toFixed(2)}L
                  </div>
                  <button
          ...
          {nav === "settings" && (
            <div style={{ maxWidth: 520 }}>
              ...
              >
                Manage your organizer profile and payouts.
              </p>
              <div
                style={{
                  background: "var(--surface)",
              ...
              >
                ...
                <button
                  className="btn btn-primary btn-sm"
                  onClick={() => toast.add("Settings Saved", "", "success")}
                >
                  Save
                </button>
              </div>
              ...
              >
                ...
                <div className="form-group">
                  ...
                  <input className="form-input" placeholder="HDFC0001234" />
                </div>
                <div className="form-group">
                ...
                <button
                  className="btn btn-primary btn-sm"
                  onClick={() => toast.add("Bank Details Saved", "", "success")}
                >
                  Save
                </button>
              </div>
    ...
    try {
      ...
      setAdminUsers(
        (u.data?.users || []).map((x) => ({
          ...
          email: x.email,
          role:
            (x.role || "user").charAt(0).toUpperCase() +
            (x.role || "user").slice(1),
          joined: x.createdAt
  ...
  if (!isAuth || user?.role !== "admin")
    return (
      <div className="page section container" style={{ textAlign: "center" }}>
        ...
        <p style={{ color: "var(--text2)" }}>
          Sign in with the admin account from your server configuration (see{" "}
          <code style={{ fontSize: 13 }}>.env</code>).
        </p>
      </div>
    );
          ...
          {nav === "overview" && (
            <div>
              ...
              <div className="stats-row" style={{ marginBottom: 32 }}>
                <StatCard
                  num={
                    adminUsers.length
                      ...
                      : adminStats
                        ? String(adminStats.users + adminStats.organizers)
                        : "—"
                  }
                  label="Users (loaded)"
                  icon="👥"
                ...
                />
                <StatCard
                  num={adminStats ? String(adminStats.organizers) : "—"}
                  label="Organizers (role)"
                  icon="🏢"
                  trend="Live API"
                />
                <StatCard
                  num={adminStats ? String(adminStats.events) : "—"}
                  label="Events"
                  icon="🎪"
                />
                <StatCard
                  num={
                    adminStats
                      ? `₹${(adminStats.grossRevenue / 100000).toFixed(1)}L`
                      : "—"
                  }
                  label="Gross Revenue"
                  icon="💰"
              ...
              >
                ...
                >
                  ...
                  {[
                    ["API Response", "98ms", "var(--green)"],
                    ["Server Uptime", "99.97%", "var(--green)"],
                    ["DB Query", "12ms", "var(--green)"],
                ...
                >
                  ...
                  {ORGANIZERS_LIST.filter((o) => o.status === "Pending").map(
                    (o) => (
                      <div
                        key={o.name}
                        className="flex-between"
                      ...
                      >
                        ...
                        <div style={{ display: "flex", gap: 6 }}>
                          <button
                            ...
                            onClick={() => toast.add("Approved", "", "success")}
                          >
                            ✓
                          </button>
                          <button
                            className="btn btn-danger btn-xs"
                            onClick={() => toast.add("Rejected", "", "error")}
                          >
                            ✗
                          </button>
                        </div>
            ...
            <div>
              <div className="flex-between mb-20">
                ...
                <div style={{ display: "flex", gap: 10 }}>
                  ...
                  <button
                    className="btn btn-primary btn-sm"
                    onClick={() => toast.add("Feature Coming Soon", "", "info")}
                  >
                    + Add User
                  </button>
                </div>
              ...
              <div className="stats-row" style={{ marginBottom: 22 }}>
                <StatCard
                  num={String(adminUsers.length)}
                  label="Loaded Users"
                  icon="👥"
                />
                <StatCard
                  num={String(
                    adminUsers.filter((u) => u.role === "Organizer").length,
                  )}
                  label="Organizers"
                  icon="🏢"
                />
                ...
                <StatCard
                  num={String(
                    adminUsers.filter((u) => u.status === "Flagged").length,
                  )}
                  label="Flagged"
                  icon="🚩"
                />
                            ...
                            <div style={{ display: "flex", gap: 6 }}>
                              ...
                              <button
                                ...
                                onClick={async () => {
                                  if (
                                    !window.confirm(
                                      "Remove this event from the platform?",
                                    )
                                  )
                                    return;
                                  try {
          ...
          {nav === "organizers" && (
            <div>
              <div className="flex-between mb-20">
                ...
                <button
                  className="btn btn-primary btn-sm"
                  onClick={() => toast.add("Invite sent", "", "success")}
                >
                  + Invite Organizer
                </button>
              </div>
                          ...
                          <div style={{ display: "flex", gap: 6 }}>
                            {o.status === "Pending" && (
                              <button
                                className="btn btn-success btn-xs"
                                onClick={() =>
                                  toast.add("Organizer Approved", "", "success")
                                }
                              >
                                Approve
                              </button>
                            ...
                            {o.status === "Suspended" ? (
                              <button
                                className="btn btn-success btn-xs"
                                onClick={() =>
                                  toast.add("Organizer Restored", "", "success")
                                }
                              >
                                Restore
                              </button>
          ...
          {nav === "settings" && (
            <div style={{ maxWidth: 580 }}>
              ...
              >
                ...
                <button
                  className="btn btn-primary btn-sm"
                  onClick={() => toast.add("Config Saved", "", "success")}
                >
                  Save Configuration
                </button>
              </div>
  ...
  return (
    <div className="page section container">
      <div className="section-header">
        ...
        <p className="section-sub">
          From discovery to entry — Eventify makes event booking effortless.
        </p>
      </div>
      <div
        style={{
      ...
      >
        {[
          [
            ...
            "🔍",
            "Discover Events",
            "Browse thousands of events — filter by city, date, category, or price.",
          ],
          [
            "2",
            "📝",
            "Create an Account",
            "Sign up free. One account to book tickets and track everything.",
          ],
          [
            "3",
            "🎟️",
            "Select & Book",
            "Choose ticket type and quantity, then proceed to checkout.",
          ],
          [
            "4",
            "💳",
            "Pay via Razorpay",
            "UPI, cards, net banking, or wallets — all SSL encrypted.",
          ],
          [
            "5",
            "📧",
            "Get Confirmation",
            "Instant email with booking ID and QR code ticket.",
          ],
          [
            "6",
            "🚪",
            "Show QR & Walk In",
            "Flash your QR at the gate. Enjoy the event!",
          ],
  ...
  return (
    <div className="page section container">
      <div style={{ maxWidth: 800, margin: "0 auto" }}>
        ...
        >
          Eventify was founded to make it effortless for anyone in India to
          discover and attend incredible events, and for organizers to manage
          them beautifully.
        </p>
        <div
          style={{
        ...
        >
          {[
            [
              "🎯",
              "Our Mission",
              "Democratize event experiences across India.",
            ],
            [
              "👁️",
              "Our Vision",
              "A future where no great event goes unnoticed.",
            ],
            [
              "💡",
              "Innovation",
              "MERN stack, real-time notifications, AI recommendations.",
            ],
            [
              "🤝",
              "Community",
              "650+ organizers, 180K+ attendees trust Eventify.",
            ],
          ].map(([icon, title, desc]) => (
            <div
              key={title}
  ...
  return (
    <div className="page section container">
      <div style={{ maxWidth: 880, margin: "0 auto" }}>
        ...
        <div
          style={{ display: "grid", gridTemplateColumns: "1fr 1.4fr", gap: 44 }}
        >
          <div>
            {[
...
function AppInner() {
  ...
  return (
    <>
      ...
      {modal && (
        <AuthModal type={modal} setModal={setModal} setPage={setPage} />
      )}
      ...
      {page === "account" && (
        <UserDashboard setPage={setPage} setModal={setModal} />
      )}
